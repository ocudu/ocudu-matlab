%ocuduSRSEstimatorUnittest Unit tests for SRS processor functions.
%   This class implements unit tests for the SRS processor functions using the
%   matlab.unittest framework. The simplest use consists in creating an object with
%       testCase = ocuduSRSEstimatorUnittest
%   and then running all the tests with
%       testResults = testCase.run
%
%   ocuduSRSEstimatorUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'srs_estimator').
%   ocuduBlockType  - The type of the tested block, including layer
%                     (i.e., 'phy/upper/signal_processors').
%
%   ocuduSRSEstimatorUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduSRSEstimatorUnittest Properties (TestParameter):
%
...
%   KTC           - Comb size.
%
%   ocuduSRSEstimatorUnittest Methods (TestTags = {'testvector'}):
%
%   testvectorGenerationCases - Generates a test vectors according to the provided
%                               parameters.
%
%   ocuduSRSEstimatorUnittest Methods (Access = protected):
%
...
classdef ocuduSRSEstimatorUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'srs_estimator'
    ...
    methods (Test, TestTags = {'testvector'})
        ...
        %   NCellID, NSlot and PRB occupation are randomly generated.
            ...
            import ocuduLib.phy.helpers.ocuduSRSValidateConfig
            ...
            srs = struct();
            while ~ocuduSRSValidateConfig(carrier, srs)
