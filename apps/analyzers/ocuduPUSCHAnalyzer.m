%ocuduPUSCHAnalyzer Analyzes a PUSCH transmission from a resource grid.
%   ocuduPUSCHAnalyzer(CARRIER, PUSCH, EXTRA, RGFILENAME, RGOFFSET, RGSIZE) analyzes the
...
%
%   % Use ocuduParseLogs to populate the carrier and PUSCH configuration objects, as
%   % well as the EXTRA struct (you will be asked to select the PUSCH entry of the logs):
%   [carrier, pusch, extra] = ocuduParseLogs
%
%   % Launch the analyzer
%   ocuduPUSCHAnalyzer(carrier, pusch, extra, 'rx_symbols.bin', 1141879, 17808)
%
...
function ocuduPUSCHAnalyzer(carrier, pusch, extra, rgFilename, rgOffset, rgSize)
    arguments
...
%% Plot analysis.
figRG = figure("Name", "ocuduPUSCHAnalyzer: Resource grid amplitude");
tiledlayout(nPorts, 1);
figChannel = figure("Name", "ocuduPUSCHAnalyzer: Channel estimate");
...
figure("Name", "ocuduPUSCHAnalyzer")
tiledlayout('flow')
