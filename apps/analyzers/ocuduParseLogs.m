%ocuduParseLogs Parses a PHY uplink channel log entry.
%   [CARRIER, PHYCH, EXTRA] = ocuduParseLogs asks the user to select a section of
%   the OCUDU gNB logs corresponding to PHY uplink channel, parses the information
...
function [carrier, phych, extra] = ocuduParseLogs
    ...
    if isPUSCH
        % If a PUSCH entry, read the TBS (recall that ocuduGNB logs it in bytes,
