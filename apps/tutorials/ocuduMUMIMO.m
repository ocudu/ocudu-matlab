...
% Multiple-Input Multiple-Output (MU-MIMO) in OCUDU. Specifically, the following
...
%
% *Remark:* Regarding channel estimation, MATLAB's behavior differs from OCUDU
% Project since MATLAB estimates the channel between antenna ports at the Tx and
% Rx sides, while OCUDU estimates the per-layer equivalent channels to the Rx
...
function [grid, infoData, infoDMRS, Hcoef] = generateRG(carrier, pusch1, pusch2, targetCodeRate, numRxPorts)
    ...
    bps1 = ocuduLib.phy.helpers.ocuduGetBitsSymbol(pusch1.Modulation);
    rv = 0;
    ...
    bps2 = ocuduLib.phy.helpers.ocuduGetBitsSymbol(pusch2.Modulation);
