%ocuduPUSCHDecoderUnittest Unit tests for PUSCH decoder functions.
%   This class implements unit tests for the PUSCH decoder functions using the
%   matlab.unittest framework. The simplest use consists in creating an object with
%      testCase = ocuduPUSCHDecoderUnittest
%   and then running all the tests with
%      testResults = testCase.run
%
%   ocuduPUSCHDecoderUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'pusch_decoder').
%   ocuduBlockType  - The type of the tested block, including layer
%                     (i.e., 'phy/upper/channel_processors/pusch').
%
%   ocuduPUSCHDecoderUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduPUSCHDecoderUnittest Properties (TestParameter):
%
...
%
%   ocuduPUSCHDecoderUnittest Methods (TestTags = {'testvector'}):
%
%   testvectorGenerationCases - Generates a test vectors according to the provided
%                               parameters.
%
%   ocuduPUSCHDecoderUnittest Methods (TestTags = {'testmex'}):
%
%   mexTest  - Tests the mex wrapper of the OCUDU PUSCH decoder.
%
%   ocuduPUSCHDecoderUnittest Methods (Access = protected):
%
...
classdef ocuduPUSCHDecoderUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'pusch_decoder'
    ...
    properties (Constant, Hidden)
        ...
        NStartBWP = 0
        NSizeBWP = ocuduPUSCHDecoderUnittest.NSizeGrid
        % MCS table.
        mcsTable = 'qam64'
    ...
    methods (Access = private)
        ...
        % Sets secondary simulation variables.
            ...
            [R, Qm] = ocuduExpandMCS(mcs, obj.mcsTable);
            obj.TargetCodeRate = R/1024;
            modulation = ocuduGetModulation(Qm);
    ...
    methods (Test, TestTags = {'testvector'})
        ...
        %   generated randomly.
            ...
            testCaseString = obj.testCaseToString(testID, ...
                {{['units::bytes{', num2str(obj.TransportBlockSize / 8), '}'],...
                    ['ldpc_base_graph_type::BG', num2str(obj.ulschInfo.BGN)], 0, ...
                    ocuduModulationFromMatlab(obj.Modulation, 'full'), Nref, ...
    ...
    methods (Test, TestTags = {'testmex'})
        ...
        %   decoded using the mex wrapper of the OCUDU C++ component. The test is considered
        %   as passed if the transmitted and received transport blocks are equal.

            import ocuduMEX.phy.ocuduPUSCHDecoder
            import ocuduTest.helpers.bitPack
            ...
            % Configure the OCUDU PUSCH decoder mex.
            ULSCHDecoder = ocuduPUSCHDecoder('MaxCodeblockSize', obj.ulschInfo.N, ...
            ...
            segmentCfg = ocuduPUSCHDecoder.configureSegment(obj.Carrier, obj.PUSCH, ...
