...
void MexFunction::method_new(ArgumentList outputs, ArgumentList inputs)
{
  ...
  if (!estimator) {
    mex_abort("Cannot create OCUDU port channel estimator.");
  }
}
...
void MexFunction::method_step(ArgumentList outputs, ArgumentList inputs)
{
  ...
  if (!estimator) {
    mex_abort("The OCUDU channel estimator was not initialized properly.");
  }
  ...
  cfg.cp                                      = matlab_to_ocudu_cyclic_prefix(in_cp.toAscii());

  cfg.scs = matlab_to_ocudu_subcarrier_spacing(static_cast<unsigned>(in_cfg["SubcarrierSpacing"][0]));
  ...
  for (unsigned i_layer = 0; i_layer != nof_layers; ++i_layer) {
    for (unsigned i_port = 0; i_port != nof_rx_ports; ++i_port) {
      ...
      ocuduvec::convert(ch_est_out_view.first(ch_estimate_view.size()), ch_estimate_view);
