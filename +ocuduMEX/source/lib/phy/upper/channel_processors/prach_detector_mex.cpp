...
void MexFunction::method_step(ArgumentList outputs, ArgumentList inputs)
{
  ...
  detector_config.restricted_set                = matlab_to_ocudu_restricted_set(restricted_set_in.toAscii());
  detector_config.root_sequence_index           = in_det_cfg["SequenceIndex"][0];
  detector_config.format                        = matlab_to_ocudu_preamble_format(format_in.toAscii());
  ...
  if (!buffer) {
    mex_abort("Cannot create OCUDU PRACH buffer.");
  }
