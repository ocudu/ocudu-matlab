...
/// PUCCH processor.
inline std::tuple<std::unique_ptr<ocudu::pucch_processor>, std::unique_ptr<ocudu::pucch_pdu_validator>>
create_pucch_processor();

/// Implements a PUCCH processor following the ocudu_mex_dispatcher template.
...
public:
  ...
  MexFunction()
  {
    ...
    // Ensure OCUDU PUCCH processor and validator were created successfully.
    if (!processor) {
      mex_abort("Cannot create OCUDU PUCCH processor.");
    }
    if (!validator) {
      mex_abort("Cannot create OCUDU PUCCH PDU validator.");
    }
...
private:
  ...
  matlab::data::TypedArray<int8_t> fill_message_fields(ocudu::span<const uint8_t> field);
  /// \brief Calls the PUCCH processor and fills the output structure array with the results.
  matlab::data::StructArray call_processor(const ocudu::resource_grid_reader& grid_reader,
                                           const matlab::data::Struct&        in_cfg,
                                           const matlab::data::StructArray&   mux_f1);

  /// A pointer to the actual PUCCH processor.
  std::unique_ptr<ocudu::pucch_processor> processor;
  /// A pointer to the PUCCH PDU validator.
  std::unique_ptr<ocudu::pucch_pdu_validator> validator;
};

std::tuple<std::unique_ptr<ocudu::pucch_processor>, std::unique_ptr<ocudu::pucch_pdu_validator>>
create_pucch_processor()
{
