...
/// PUSCH demodulator.
inline std::unique_ptr<ocudu::pusch_demodulator> create_pusch_demodulator();

/// Implements a PUSCH demodulator following the ocudu_mex_dispatcher template.
...
private:
  ...
  ///
  /// This method creates the OCUDU PUSCH demodulator object used by MEX wrapper, with the given equalization strategy.
  ///
  ...
  std::unique_ptr<ocudu::pusch_demodulator> demodulator = nullptr;
};

inline std::unique_ptr<ocudu::pusch_demodulator>
create_pusch_demodulator(ocudu::channel_equalizer_algorithm_type eq_type)
{
  ...
  std::shared_ptr<pusch_demodulator_factory> pusch_demod_factory = create_pusch_demodulator_factory_sw(
      equalizer_factory, transform_precod_factory, demod_factory, nullptr, prg_factory, MAX_NOF_PRBS);
