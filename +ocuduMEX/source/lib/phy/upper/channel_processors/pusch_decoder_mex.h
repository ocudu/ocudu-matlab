...
#include "ocudu_matlab/support/memento.h"
...
/// PUSCH decoder.
inline std::unique_ptr<ocudu::pusch_decoder> create_pusch_decoder();

/// Implements a PUSCH decoder following the ocudu_mex_dispatcher template.
class MexFunction : public ocudu_mex_dispatcher
{
  ...
  public:
    ...
    explicit pusch_memento(std::unique_ptr<ocudu::rx_buffer_pool_controller> p) : pool(std::move(p)) {}
    ...
    ocudu::unique_rx_buffer
    retrieve_softbuffer(const ocudu::trx_buffer_identifier& id, unsigned nof_codeblocks, bool is_new_data);

  private:
    /// Pointer to the softbuffer pool stored in the memento.
    std::unique_ptr<ocudu::rx_buffer_pool_controller> pool;
  };
...
public:
  ...
  MexFunction()
  {
    // Ensure OCUDU PUSCH decoder was created successfully.
    if (!decoder) {
      mex_abort("Cannot create OCUDU PUSCH decoder.");
    }
...
private:
  ...
  ocudu::unique_rx_buffer
  retrieve_softbuffer(uint64_t key, const ocudu::trx_buffer_identifier& id, unsigned nof_codeblocks, bool is_new_data);
  ...
  ///   - A one-dimensional structure with fields (see also ocudu::rx_softbuffer_pool_description):
  ...
  std::unique_ptr<ocudu::pusch_decoder> decoder = create_pusch_decoder();

  /// A container for pusch_memento objects.
  memento_storage<pusch_memento> storage;
};

std::unique_ptr<ocudu::pusch_decoder> create_pusch_decoder()
{
