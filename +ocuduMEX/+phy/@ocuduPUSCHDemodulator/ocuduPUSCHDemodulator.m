%ocuduPUSCHDemodulator MATLAB interface to OCUDU PUSCH demodulator.
%   User-friendly interface to the OCUDU PUSCH demodulator class, which is wrapped
...
%
%   ocuduPUSCHDemodulator properties (nontunable):
%
...
classdef ocuduPUSCHDemodulator < matlab.System
    properties (Nontunable)
        EqualizerStrategy (1, :) char {mustBeMember(EqualizerStrategy, {'ZF', 'MMSE'})} = 'ZF'
    end

    methods
        function obj = ocuduPUSCHDemodulator(varargin)
    ...
    methods (Access = protected)
        ...
        function schSoftBits = stepImpl(obj, rxSymbols, cest, noiseVar, pusch, puschIndices, puschDMRSIndices, rxPorts)
            arguments
                obj               (1, 1)        ocuduMEX.phy.ocuduPUSCHDemodulator
                rxSymbols         (:, 14, :)    double {ocuduTest.helpers.mustBeResourceGrid}
                cest              (:, 14, :, :) double {ocuduTest.helpers.mustBeResourceGrid(cest, MultiLayer=1)}
            ...
            ceSize = size(cest);
            assert(all(gridSize(1:2) == ceSize(1:2)), 'ocudu_matlab:ocuduPUSCHDemodulator', ...
                'Resource grid and channel estimates sizes do not match.');
            if (numel(gridSize) > 2)
                assert(numel(ceSize) > 2, 'ocudu_matlab:ocuduPUSCHDemodulator', ...
                    'Resource grid and channel estimates sizes do not match.');
                assert(all(gridSize(3) == ceSize(3)), 'ocudu_matlab:ocuduPUSCHDemodulator', ...
                    'Resource grid and channel estimates sizes do not match.');
                assert(numel(rxPorts) <= gridSize(3), 'ocudu_matlab:ocuduPUSCHDemodulator', ...
                    'The number of PUSCH ports, %d, cannot be larger than the number of Rx antenna ports, %d.', ...
                    numel(rxPorts), gridSize(3));
            else
                assert(numel(ceSize) == 2, 'ocudu_matlab:ocuduPUSCHDemodulator', ...
                    'Resource grid and channel estimates sizes do not match.');
                assert(isscalar(rxPorts), 'ocudu_matlab:ocuduPUSCHDemodulator', ...
                    'The number of PUSCH ports, %d, cannot be larger than the number of Rx antenna ports, 1.', ...
                    numel(rxPorts));
            end

            assert(size(puschIndices, 2) == pusch.NumLayers, 'ocudu_matlab:ocuduPUSCHDemodulator', ...
                'The number of columns of puschIndices should be equal to the number of layers.');
            assert(size(puschDMRSIndices, 2) == pusch.NumLayers, 'ocudu_matlab:ocuduPUSCHDemodulator', ...
            ...
            for iLayer = 2:pusch.NumLayers
                assert(all(puschIndices(:, 1) == puschIndices(:, iLayer) - gridSize(1) * gridSize(2) * (iLayer - 1)), ...
                    'ocudu_matlab:ocuduPUSCHDemodulator', ...
            ...
            PUSCHDemConfig = struct( ...
                ...
                'RxPorts', rxPorts, ...
                'NumOutputLLR', numel(puschIndices) * ocuduLib.phy.helpers.ocuduGetBitsSymbol(pusch.Modulation));
...
end % of classdef ocuduPUSCHDemodulator < matlab.System
