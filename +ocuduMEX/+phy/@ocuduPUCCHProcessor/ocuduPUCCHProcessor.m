%ocuduPUCCHProcessor retrieves UCI messages from a PUCCH transmission.
%   User-friendly interface for processing a PUCCH transmission via the MEX static
%   method pucch_processor_mex, which calls OCUDU pucch_processor.
%
%   PROCESSOR = ocuduPUCCHProcessor creates the PUCCH processor object PROCESSOR.
%
%   ocuduPUCCHProcessor Methods:
%
...
%   'MaxCodeRate'  - Maximum code rate (float). Default 0.25.
...
classdef ocuduPUCCHProcessor < matlab.System
    methods (Access = protected)
        function uci = stepImpl(obj, carrierConfig, pucchConfig, rxGrid, uciSizes)
            arguments
                obj                   (1, 1) ocuduMEX.phy.ocuduPUCCHProcessor
                ...
                uciSizes.MaxCodeRate  (1, 1) double {mustBeNonnegative}                = 0.25
            ...
            if isa(pucchConfig, 'nrPUCCH0Config')
                ...
                assert(isempty(uciSizes.MuxFormat1), 'OCUDU-matlab:ocuduPUCCHProcessor', 'Input MuxFormat1 should be empty for PUCCH Format 0');
            ...
            elseif isa(pucchConfig, 'nrPUCCH2Config')
                ...
                mexConfig = struct(...
                    ...
                    'MaxCodeRate', uciSizes.MaxCodeRate, ...
                ...
                assert(isempty(uciSizes.MuxFormat1), 'OCUDU-matlab:ocuduPUCCHProcessor', 'Input MuxFormat1 should be empty for PUCCH Format 2');
            elseif isa(pucchConfig, 'nrPUCCH3Config')
                ...
                assert(isempty(uciSizes.MuxFormat1), 'OCUDU-matlab:ocuduPUCCHProcessor', 'Input MuxFormat1 should be empty for PUCCH Format 3');
            else
                if ~isempty(pucchConfig.NID)
                    nid = pucchConfig.NID;
                end
                ...
                assert(isempty(uciSizes.MuxFormat1), 'OCUDU-matlab:ocuduPUCCHProcessor', 'Input MuxFormat1 should be empty for PUCCH Format 4');
            end
...
end % of classdef ocuduPUCCHProcessor < matlab.System
