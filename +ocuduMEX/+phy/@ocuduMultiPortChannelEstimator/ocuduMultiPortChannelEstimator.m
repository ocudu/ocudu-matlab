%ocuduMultiPortChannelEstimator estimates a SIMO channel.
...
%   calls OCUDU port_channel_estimator for each port and combines the outputs.
%
%   CHESTIMATOR = ocuduMultiPortChannelEstimator creates the SIMO channel estimator
%   object CHESTIMATOR.
%
%   CHESTIMATOR = ocuduMultiPortChannelEstimator(NAME, VALUE, ...) creates a SIMO channel estimator
%   with properties (see below) set according to the NAME-VALUE pairs.
%
%   ocuduMultiPortChannelEstimator Methods:
%
...
classdef ocuduMultiPortChannelEstimator < matlab.System
    ...
    methods
        function obj = ocuduMultiPortChannelEstimator(varargin)
    ...
    methods (Access = protected)
        ...
        function [channelEst, noiseEst, extra] = stepImpl(obj, rxGrid, ...
            ...
            arguments
                obj                      (1, 1)     ocuduMEX.phy.ocuduMultiPortChannelEstimator
                rxGrid                   (:, 14, :) double {ocuduTest.helpers.mustBeResourceGrid}
            ...
            assert(symbolAllocation(1) < 14, 'ocudu_matlab:ocuduMultiPortChannelEstimator', 'First allocated symbol out of range.');
            lastSymbol = sum(symbolAllocation) - 1;
            assert(lastSymbol < 14, 'ocudu_matlab:ocuduMultiPortChannelEstimator', 'Last allocated symbol out of range.');

            [nPilots, nLayers] = size(refSym);
            assert(nLayers <= 4, 'ocudu_matlab:ocuduMultiPortChannelEstimator', ...
                'Currently, max 4 layers supported, provided %d.', nLayers);
            assert(nPilots == numel(refInd(:, 1)), 'ocudu_matlab:ocuduMultiPortChannelEstimator', ...
                ['The number of pilots per layer %d and the number of pilot resources %d ', ...
                 'do not match.'], nPilots, numel(refInd));
            assert(nLayers == size(refInd, 2), 'ocudu_matlab:ocuduMultiPortChannelEstimator', ...
            ...
            if nLayers > 1
                assert(all(refInd(:, 1) == refInd(:, 2) - nREs), ...
                    'ocudu_matlab:ocuduMultiPortChannelEstimator', ...
    ...
    methods (Access = private)
        ...
        % Implementation of the step method that uses the MEX.
            ...
            if size(refInd, 2) == 2
                assert(all(refInd(:, 2) - refInd(:, 1) == 1), 'ocudu_matlab:ocuduMultiPortChannelEstimator', ...
            ...
            nLayersMEX = 4;
            assert(nLayers <= nLayersMEX, 'ocudu_matlab:ocuduMultiPortChannelEstimator', ...
        ...
        % Implementation of the step method that uses OCUDU matlab implementation.
            ...
            for iPort = 1:nPorts
                [channelEst(:, :, iPort, :), noiseEstTmp, rsrpTmp, epreTmp, taTmp, cfoTmp] = ...
                    ocuduChannelEstimator(approxbf16(rxGrid(:, :, iPort)), pilots, config.BetaScaling, hop1, hop2, configNew);
...
end % of classdef ocuduPortChannelEstimator < matlab.System
