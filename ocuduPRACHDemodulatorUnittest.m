%ocuduPRACHDemodulatorUnittest Unit tests for PRACH waveform demodulator.
...
%       testCase = ocuduPRACHDemodulatorUnittest
%   and then running all the tests with
%       testResults = testCase.run
%
%   ocuduPRACHDemodulatorUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'ofdm_prach_demodulator').
%   ocuduBlockType  - The type of the tested block, including layer
%                     (i.e., 'phy/lower/modulation').
%
%   ocuduPRACHDemodulatorUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduPRACHDemodulatorUnittest Properties (TestParameter):
%
...
%                               parameters.
%
%   ocuduPRACHDemodulatorUnittest Methods (Access = protected):
%
...
classdef ocuduPRACHDemodulatorUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'ofdm_prach_demodulator'
        ...
        %   PreambleFormat and PreambleIndex are generated randomly.
            ...
            for TimeIndex = 1:prach.NumTimeOccasions
                for FrequencyIndex = 1:NumFreqOccasions
                    ...
                    [occasionTmp, gridset, info] = ocuduPRACHgenerator(carrier, prach);

                    % Pad occasion with zeros to match the waveform size.
                    occasion = [occasionTmp; zeros(numel(waveform) - numel(occasionTmp), 1)];

                    % Try to demodulate.
                    demodulated = ocuduPRACHdemodulator(carrier, prach, gridset.Info, occasion, info);
            ...
            % The OCUDU PRACH demodulator uses the isometric DFT, such that
            ...
            % Convert the sampling rate to OCUDU format.
            ocuduSampleRateString = sprintf('sampling_rate::from_MHz(%.2f)', gridset.Info.SampleRate / 1e6);
            ...
            ocuduPRACHFormat = sprintf('to_prach_format_type("%s")', PreambleFormat);

            % Slot configuration.
            slotConfig = {log2(carrier.SubcarrierSpacing/15), info.NPRACHSlot};

            % ocudu PRACH configuration
            ocuduPRACHConfig = {...
                slotConfig, ...                       % slot
            ...
            % test context
            ocuduTestContext = {
                ocuduSampleRateString, ... % srate
                ocuduPRACHConfig, ...      % config
                };

            % Generate the test case entry
            testCaseString = testCase.testCaseToString(TestID, ...
                ocuduTestContext, true, '_test_input', '_test_output');

            % Add the test to the file header
            testCase.addTestToHeaderFile(testCase.headerFileID, testCaseString);
        end % of function testvectorGenerationCases
    end % of methods (Test, TestTags = {'testvector'})
end % of classdef ocuduPRACHDemodulatorUnittest
