%ocuduPDCCHModulatorUnittest Unit tests for PDCCH modulator functions.
%   This class implements unit tests for the PDCCH modulator functions using the
%   matlab.unittest framework. The simplest use consists in creating an object with
%       testCase = ocuduPDCCHModulatorUnittest
%   and then running all the tests with
%       testResults = testCase.run
%
%   ocuduPDCCHModulatorUnittest Properties (Constant):
%
...
%
%   ocuduPDCCHModulatorUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduPDCCHModulatorUnittest Properties (TestParameter):
%
%   Duration         - CORESET Duration.
%   CCEREGMapping    - CCE-to-REG mapping.
%   AggregationLevel - PDCCH aggregation level.
%
%   ocuduPDCCHModulatorUnittest Methods (TestTags = {'testvector'}):
%
...
classdef ocuduPDCCHModulatorUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'pdcch_modulator'
    ...
    methods (Access = protected)
        ...
        function initializeClassImpl(obj)
            obj.randomizeTestvector = randperm(ocuduPDCCHModulatorUnittest.nofNCellID);
            obj.randomizeSlot = randi([1, 10], 1, ocuduPDCCHModulatorUnittest.nofNCellID);
        end
    ...
    methods (Test, TestTags = {'testvector'})
        ...
        %   RNTI and codeword.
            ...
            if (isAggregationOK && isCCEREGMappingOK)
                ...
                [PDCCHsymbols, symbolIndices] = ocuduPDCCHmodulator(codeWord, carrier, pdcch, DMRSScramblingID, rnti);
...
end % of classdef ocuduPDCCHModulatorUnittest< ocuduTest.ocuduBlockUnittest
