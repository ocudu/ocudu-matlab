%ocuduDemodulationMapperUnittest Unit tests for the modulation mapper functions.
%   This class implements unit tests for the demodulation mapper functions using the
%   matlab.unittest framework. The simplest use consists in creating an object with
%       testCase = ocuduDemodulationMapperUnittest
%   and then running all the tests with
%       testResults = testCase.run
%
%   ocuduDemodulationMapperUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'demodulation_mapper').
%   ocuduBlockType  - The type of the tested block, including layer
%                     (i.e., '/phy/upper/channel_modulation').
%
%   ocuduDemodulationMapperUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduDemodulationMapperUnittest Properties (TestParameter):
%
%   Modulation - Modulation scheme (see extended documentation for details).
%   nSymbols  - Number of modulated output symbols (257, 997).
%
%   ocuduDemodulationMapperUnittest Methods (TestTags = {'testvector'}):
%
%   testvectorGenerationCases - Generates a test vector for the given modulation
%                               scheme and number of symbols.
%
%   ocuduDemodulationMapperUnittest Methods (Access = protected):
%
...
classdef ocuduDemodulationMapperUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'demodulation_mapper'
    ...
    methods (Test, TestTags = {'testvector'})
        ...
        %   for the given number of symbols NSYMBOLS and modulation scheme and MODSCHEME.
            ...
            % Demodulate (note that ocuduDemodulator returns integer values
            % that can be directly assigned to int8_t variables).
            softBits = ocuduDemodulator(noisySymbols, Modulation, noiseVar);
...
end % of classdef ocuduDemodulationMapperUnittest
