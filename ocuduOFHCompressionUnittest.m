%ocuduOFHCompressionUnittest Unit tests for the IQ data compression used in OFH implementation.
%   This class implements unit tests for the IQ data compression functions using the
%   matlab.unittest framework. The simplest use consists in creating an object with
%       testCase = ocuduOFHCompressionUnittest
%   and then running all the tests with
%       testResults = testCase.run
%
%   ocuduOFHCompressionUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'iq_compression').
%   ocuduBlockType  - The type of the tested block, including layer
%                     (i.e., '/ru/compression').
%
%   ocuduOFHCompressionUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduOFHCompressionUnittest Properties (TestParameter):
%
%   method    - OFH compression method.
%   nPRBs     - Number of PRBs to be compressed.
%   cIQwidth  - Compressed IQ samples bit-width.
%
%   ocuduOFHCompressionUnittest Methods (TestTags = {'testvector'}):
%
%   testvectorGenerationCases - Generates a test vector for the given
%                               compression method and number of PRBs.
%
%   ocuduOFHCompressionUnittest Methods (Access = protected):
%
...
classdef ocuduOFHCompressionUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'ofh_compression'

        %Type of the tested block, including layers.
        ocuduBlockType = 'ofh/compression'
    end
    ...
    methods (Access = protected)
        ...
        %addTestIncludesToHeaderFile Adds include directives to the test header file.
            ...
            fprintf(fileID, '#include "ocudu/%s/%s"\n', ...
                obj.ocuduBlockType, 'compression_params.h');
    ...
    methods (Test, TestTags = {'testvector'})
        ...
        %   compressed IQ width CIQWIDTH.
            ...
            import ocuduTest.helpers.writeInt16File
            import ocuduTest.helpers.writeComplexFloatFile
            import ocuduTest.helpers.approxbf16
            import ocuduLib.ofh.compression.ocuduCompressor
            ...
            if (strcmp(method, 'none'))
                ...
                [cIQData, cParam] = ocuduCompressor(iqDataQuantized, method, cIQwidth);
            end
...
end % of classdef ocuduOFHCompressionUnittest
