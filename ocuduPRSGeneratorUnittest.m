%ocuduPRSGeneratorUnittest Unit tests for PRS generator functions.
%   This class implements unit tests for the PRS generator functions using the
%   matlab.unittest framework. The simplest use consists in creating an object with
%       testCase = ocuduPRSGeneratorUnittest
%   and then running all the tests with
%       testResults = testCase.run
%
%   ocuduPRSGeneratorUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'prs_generator').
%   ocuduBlockType  - The type of the tested block, including layer
%                     (i.e., 'phy/upper/signal_processors/prs').
%
%   ocuduPRSGeneratorUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduPRSGeneratorUnittest Properties (TestParameter):
%
%   Numerology          - Defines the subcarrier spacing (0, 1).
%   DurationAndCombSize - Combinations of duration and comb size.
%
%   ocuduPRSGeneratorUnittest Methods (TestTags = {'testvector'}):
%
%   testvectorGenerationCases - Generates a test vector according to the provided
%                               parameters.
%
%   ocuduPRSGeneratorUnittest Methods (Access = protected):
%
...
classdef ocuduPRSGeneratorUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'prs_generator'

        %Type of the tested block.
        ocuduBlockType = 'phy/upper/signal_processors/prs'
    ...
    properties (ClassSetupParameter)
        %Path to results folder (old 'prs_generator' tests will be erased).
    ...
    methods (Access = protected)
        ...
        %addTestIncludesToHeaderFile Adds include directives to the test header file.
            fprintf(fileID, '#include "ocudu/phy/upper/signal_processors/prs/prs_generator.h"\n');
            fprintf(fileID, '#include "ocudu/phy/upper/signal_processors/prs/prs_generator_configuration.h"\n');
    ...
    methods (Test, TestTags = {'testvector'})
        ...
        % Convert parameters to OCUDU types.
...
end % of classdef ocuduPRSGeneratorUnittest
