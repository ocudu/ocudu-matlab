%ocuduBlockUnittest Unit test template for OCUDU blocks (Abstract class).
%   Common functionalities shared by all OCUDU unit tests. Derives from
%   'matlab.unittest.TestCase'.
%
%   ocuduBlockUnittest Properties (Abstract, Constant):
%
%   ocuduBlock      - Name of the tested block (e.g., 'pbch_modulator').
%   ocuduBlockType  - Type of the tested block, including layer
%                     (e.g., 'phy/upper/channel_processors').
%
%   ocuduBlockUnittest Properties (Abstract, ClassSetupParameter):
%
%   outputPath  - Path to results folder (contents may be erased).
%
%   ocuduBlockUnittest Methods (TestClassSetup):
%
%   initializeClass  - Test class setup.
%
%   ocuduBlockUnittest Methods (Abstract, Access = protected):
%
%   addTestIncludesToHeaderFile    - Adds include directives to the test header file.
%   addTestDefinitionToHeaderFile  - Adds details (e.g., type/variable declarations)
%                                    to the test header file.
%
%   ocuduBlockUnittest Methods (Access = protected):
%
...
%
%   ocuduBlockUnittest Methods (Access = private):
%
...
%
%   ocuduBlockUnittest Methods (Static, Access = protected):
%
...
classdef ocuduBlockUnittest < matlab.unittest.TestCase
    ...
    properties (Hidden)
        %Path of the tested block relative to the OCUDU include root folder,
    ...
    methods (TestClassSetup)
        ...
        %   header file for the test vectors. Initializes the random generator.
            tmp = obj.ocuduBlockType;
            tmp(tmp == filesep) = '_';
            obj.pathInRepo = upper([tmp, '_', obj.ocuduBlock]);
    ...
    methods (Access = protected)
        ...
        %   method is meant for blocks of type "phy/upper/signal_processors".
            ...
            fprintf(fileID, '#include "ocudu/%s/%s.h"\n', ...
                obj.ocuduBlockType, obj.ocuduBlock);
        ...
        %   method is meant for blocks of type "phy/upper/channel_processors".
            ...
            fprintf(fileID, '#include "ocudu/%s/%s.h"\n', ...
                obj.ocuduBlockType, obj.ocuduBlock);
            fprintf(fileID, '#include "ocudu/support/file_vector.h"\n');
            if ~endsWith(obj.ocuduBlock, '_encoder')
        ...
        %   "phy/upper/channel_processors".
            ...
            fprintf(fileID, 'struct test_case_t {\n');
            if endsWith(obj.ocuduBlock, 'encoder')
                if strcmp(obj.ocuduBlock, 'pbch_encoder')
        ...
        %generateTestID Generates an identifier for the current test.
            filenameTemplateIn = sprintf('%s/%s_test_input*', obj.tmpOutputPath, obj.ocuduBlock);
            filenameTemplateOut = sprintf('%s/%s_test_output*', obj.tmpOutputPath, obj.ocuduBlock);
            if nargin == 1
                specialFilename = 'void';
            end
            filenameTemplateSpec = sprintf('%s/%s%s*', obj.tmpOutputPath, obj.ocuduBlock, specialFilename);
        ...
        %   the data of all variables VAR1, VAR2, ...
            ...
            filename = [obj.ocuduBlock suffix num2str(testID) '.dat'];
    ...
    methods (Access = private)
        ...
        %packResults(OBJ) packs all generated test vectors in a single '.tar.gz' file.
            ...
            system(sprintf('rm -rf %s/%s*.dat', obj.tmpOutputPath, obj.ocuduBlock));
        end
...
end % of classdef ocuduBlockUnittest
