%ocuduPUSCHDemodulatorUnittest Unit tests for PUSCH symbol demodulator functions.
%   This class implements unit tests for the PUSCH symbol demodulator functions using the
%   matlab.unittest framework. The simplest use consists in creating an object with
%      testCase = ocuduPUSCHDemodulatorUnittest
%   and then running all the tests with
%      testResults = testCase.run
%
%   ocuduPUSCHDemodulatorUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'pusch_demodulator').
%   ocuduBlockType  - The type of the tested block, including layer
%                     (i.e., 'phy/upper/channel_processors/pusch').
%
%   ocuduPUSCHDemodulatorUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduPUSCHDemodulatorUnittest Properties (TestParameter):
%
...
%                               parameters.
%
%   ocuduPUSCHDemodulatorUnittest Methods (TestTags = {'testmex'}):
%
%   mexTest  - Tests the MEX-based implementation of the PUSCH demodulator.
%
%   ocuduPUSCHDemodulatorUnittest Methods (Access = protected):
%
...
classdef ocuduPUSCHDemodulatorUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'pusch_demodulator'

        %Type of the tested block.
        ocuduBlockType = 'phy/upper/channel_processors/pusch'
    ...
    methods (Access = protected)
        ...
        %addTestIncludesToHeaderFile Adds include directives to the test header file.
            ...
            fprintf(fileID, '#include "ocudu/phy/upper/channel_processors/pusch/pusch_demodulator.h"\n');
    ...
    methods (Test, TestTags = {'testvector'})
        ...
        %   DMRSConfigurationType, Modulation, and ChannelSize.
            ...
            % Equalize.
            [eqSymbols, eqNoise] = ocuduChannelEqualizer(approxbf16(rxSymbols), approxbf16(cePusch), 'ZF', noiseVar, 1.0);

            % Revert transform precoding if it is present.
            if obj.pusch.TransformPrecoding
                numPRB = length(obj.pusch.PRBSet);
                [eqSymbols, eqNoise] = ocuduTransformDeprecode(eqSymbols, eqNoise, numPRB, NumLayers);
            end
            ...
            % Soft demapping.
            softBits = ocuduDemodulator(eqSymbols, obj.pusch.Modulation, eqNoise);
    ...
    methods (Test, TestTags = {'testmex'})
        ...
        %mexTest  Tests the mex wrapper of the OCUDU PUSCH demodulator.
        ...
        %   OCUDU C++ component. The test is considered as passed if the
        %   recovered soft bits are coinciding with those originally transmitted.
            ...
            % Initialize the OCUDU PUSCH demodulator mex.
            PUSCHDemodulator = ocuduPUSCHDemodulator;
            ...
            % iv) Compare OCUDU and MATLAB results.
...
end % of classdef ocuduPUSCHDemodulatorUnittest
