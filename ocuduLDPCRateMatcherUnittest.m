%ocuduLDPCRateMatcherUnittest Unit tests for the LDPC rate matcher.
...
%       testCase = ocuduLDPCRateMatcherUnittest
%   and then running all the tests with
%       testResults = testCase.run
%
%   ocuduLDPCRateMatcherUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'ldpc_rate_matcher').
...
%                     (i.e., 700).
%
%   ocuduLDPCRateMatcherUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduLDPCRateMatcherUnittest Properties (TestParameter):
%
...
%
%   ocuduLDPCRateMatcherUnittest Methods (TestTags = {'testvector'}):
%
%   testvectorGenerationCases - Generates a test vector for the given configuration.
%
%   ocuduLDPCRateMatcherUnittest Methods (Access = protected):
%
...
classdef ocuduLDPCRateMatcherUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'ldpc_rate_matcher'
    ...
    methods (Test, TestTags = {'testvector'}, ParameterCombination = 'pairwise')
        ...
        %   rate-matched length, redundancy version, modulation scheme and LBRM flag.
            ...
            % OCUDU rate matcher works on a codeblock basis and its transparent
...
end % of classdef ocuduLDPCRateMatcherUnittest
