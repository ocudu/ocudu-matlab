%ocuduLDPCSegmenterUnittest Unit tests for the LDPC segmentation functions.
%   This class implements unit tests for the LDPC segmentation functions using the
%   matlab.unittest framework. The simplest use consists in creating an object with
%       testCase = ocuduLDPCSegmenterUnittest
%   and then running all the tests with
%       testResults = testCase.run
%
%   ocuduLDPCSegmenterUnittest Properties (Constant):
%
%   ocuduBlock      - The tested block (i.e., 'ldpc_segmenter').
%   ocuduBlockType  - The type of the tested block, including layer
%                     (i.e., '/phy/upper/channel_coding/ldpc').
%
%   ocuduLDPCSegmenterUnittest Properties (ClassSetupParameter):
%
%   outputPath - Path to the folder where the test results are stored.
%
%   ocuduLDPCSegmenterUnittest Properties (TestParameter):
%
%   cases  - Configurations to test.
%
%   ocuduLDPCSegmenterUnittest Methods (TestTags = {'testvector'}):
%
%   testvectorGenerationCases - Generates a test vector for the given configuration.
%
%   ocuduLDPCSegmenterUnittest Methods (Access = protected):
%
...
classdef ocuduLDPCSegmenterUnittest < ocuduTest.ocuduBlockUnittest
    properties (Constant)
        %Name of the tested block.
        ocuduBlock = 'ldpc_segmenter'
    ...
    methods (Test, TestTags = {'testvector'})
        ...
        %testvectorGenerationCases Generates a test vector for the given configuration.
            ...
            import ocuduLib.phy.upper.channel_coding.ldpc.ocuduLDPCsegmenter
            ...
            segments = ocuduLDPCsegmenter(transBlock, cases.bg);
...
end % of classdef ocuduLDPCSegmenterUnittest
