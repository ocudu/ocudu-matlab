%ocuduChannelEqualizer 5G NR MIMO Channel Equalizer.
%
%   [EQSYMBOLS, EQNOISEVARS] = ocuduChannelEqualizer(RXSYMBOLS, CHESTS, EQTYPE, NOISEVAR, TXSCALING)
...
function [eqSymbols, eqNoiseVars] = ocuduChannelEqualizer(rxSymbols, chEsts, eqType, noiseVar, txScaling)
...
if strcmp(eqType, 'MMSE')
    ...
    assert(all(eqNoiseVars > 0, 'all'), 'ocudu_matlab:ocuduChannelEqualizer', ...
...
elseif strcmp(eqType, 'ZF')
    ...
    assert(all(abs(eqNoiseVars - noiseVar ./ csi) ./ eqNoiseVars < 1e-6, 'all'), ...
        'ocudu_matlab::ocuduChannelEqualizer', ...
