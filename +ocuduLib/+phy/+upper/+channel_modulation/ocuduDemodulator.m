%ocuduDemodulator Soft demodulator for NR constellations.
%   LLRSQ = ocuduDemodulator(SYMBOLS, SCHEME) computes the log-likelihood ratios LLRSQ
...
%
%   LLRSQ = ocuduDemodulator(..., NOISEVAR) specifies the noise variance for the input
...
function llrsq = ocuduDemodulator(symbols, scheme, noiseVar)
    arguments
        symbols (:, 1) double
...
function softBitsQuant = quantize(softBits, mod)
    ...
    switch mod
        ...
        case '256QAM'
            rangeLimitFloat = 20;
        otherwise
            error('ocuduDemodulator:Unknown constellation.');
    end
