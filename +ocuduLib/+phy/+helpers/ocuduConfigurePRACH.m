%ocuduConfigurePRACH Generates a physical random access channel (PRACH) configuration object.
%   PRACH = ocuduConfigurePRACH(PREAMBLEFORMAT, Name, Value, Name, Value, ...) returns a PRACH
...
function prach = ocuduConfigurePRACH(preambleFormat, prachParams)
    arguments
...
function prach = setPreambleFormat(prach, preambleFormat)
    import ocuduLib.phy.helpers.ocuduSelectPRACHConfigurationIndex

    % Select configuration index according to the duplex mode and preamble
    % format.
    prach.ConfigurationIndex = ocuduSelectPRACHConfigurationIndex(prach.FrequencyRange, prach.DuplexMode, preambleFormat);
